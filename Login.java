			...
			Connection con=ForConnection.con();
				...
				while(rs.next()) {
					...
					if( name.equals(rs.getString(1)) && pass.equals(rs.getString(2))){
						...
						url="select * from personal where id='"+name+"'";
						ResultSet rs1=st.executeQuery(url);
						if(rs1.next()){
							RequestDispatcher rd=request.getRequestDispatcher("view-status.html");
			...
			Connection con=ForConnection.con();
				...
				while(rs1.next()) {
					...
					if( name1.equals(rs1.getString(1)) && pass1.equals(rs1.getString(2))){
						...
						System.out.println("emp_idd"+name1);
						Cookie ck=new Cookie("emp_userid",name1);
